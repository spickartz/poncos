...
std::string cgroup_controller::generate_command(const jobT &job, size_t counter, const execute_config &config) const {
	...
	for (size_t i = 0; i < config.size(); ++i) {
		...
		if (i + 1 < config.size() && config[i].first == config[i + 1].first) {
			++i;
			host_lists[SLOTS].push_back(machines[config[i].first]);
			host_lists[SLOTS].push_back(machines[config[i].first]);
			hosts_per_slot[SLOTS] += 2;
			continue;
		}
		host_lists[config[i].second].push_back(machines[config[i].first]);
	...
	// we should only use either of the slots or all of them
	// TODO: do we want to handle unbalanced jobs?
	assert(std::count(hosts_per_slot, hosts_per_slot + SLOTS + 1, 0) == SLOTS);
	...
	for (size_t i = 0; i <= SLOTS; ++i) {
		...
		for (const auto &host : host_lists[i]) {
			hosts_file << host + ":";
			hosts_file << job.nprocs / hosts_per_slot[i];
			hosts_file << std::endl;
		}
	}
	hosts_file.close();
